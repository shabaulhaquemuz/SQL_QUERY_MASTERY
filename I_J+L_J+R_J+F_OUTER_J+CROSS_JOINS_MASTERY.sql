...
SELECT * FROM student_regex s CROSS JOIN course_regex c;

-- 2. Show every possible student-faculty pairing.
SELECT * FROM student_regex  CROSS JOIN faculty_regex;

-- 3. Create a matrix of all students with all courses (Cartesian product).
SELECT * FROM student_regex CROSS JOIN course_regex;

-- 4. Display all possible course-faculty combinations.
SELECT * FROM course_regex CROSS JOIN faculty_regex;



-- 6. Show how many total combinations exist between students and courses.
SELECT COUNT(*) AS total_combination FROM student_regex CROSS JOIN course_regex;

-- 7. Create every possible pairing of students with faculty members.
SELECT student_name, faculty_name FROM student_regex CROSS JOIN faculty_regex;
SELECT s.student_name, f.faculty_name FROM student_regex s CROSS JOIN faculty_regex f;

-- 8. List all possible enrollments if every student could take every course.
SELECT s.student_name, c.course_name FROM student_regex s CROSS JOIN course_regex c;
...
SELECT s.email AS student_email, f.email AS faculty_email FROM student_regex s CROSS JOIN faculty_regex f;
